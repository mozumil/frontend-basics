...
function findBySpecialization(spec) {
  const result = employees.filter(e => e.specialization.toLowerCase() === spec.toLowerCase());
  const display = result
    .map(e => `<p>${e.id}: ${e.name} - ${e.specialization} - $${e.salary}</p>`)
    .join('');
  document.getElementById('employeesDetails').innerHTML = display || 'No employees found with that specialization';
}
